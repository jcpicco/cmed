...
import com.medcentermanager.app.mappers.patients.*;
...
class PatientControllerTest {
    ...
    class CreatePatientTests {
        ...
        @Test
        @DisplayName("Should create patient without phone")
        void createPatient_withoutPhone_returnsCreatedPatient() throws Exception {
            PatientCreateDto noPhoneDto = PatientCreateDto.builder()
                    .name("Jane")
                    .lastName("NoPhone")
                    .dni("12345678B")
                    .birthDate(LocalDate.of(1990, 1, 1))
            ...
            Patient noPhonePatient = new Patient();
            noPhonePatient.setId(UUID.randomUUID());
            noPhonePatient.setName("Jane");
            noPhonePatient.setLastName("NoPhone");
            noPhonePatient.setDni("12345678B");
            noPhonePatient.setBirthDate(LocalDate.of(1990, 1, 1));

            when(patientService.create(any(PatientCreateDto.class))).thenReturn(patientDtoMapper.toDto(noPhonePatient));

            mockMvc.perform(post("/api/patients")
                            .contentType(MediaType.APPLICATION_JSON)
                            .content(objectMapper.writeValueAsString(noPhoneDto)))
                    .andExpect(status().isCreated())
        ...
        @Test
        @DisplayName("Should create patient without birthDate")
        void createPatient_withoutBirthDate_returnsCreatedPatient() throws Exception {
            PatientCreateDto noBirthDateDto = PatientCreateDto.builder()
                    .name("Jane")
                    .lastName("NoBirthDate")
                    .phone("123456789")
                    .dni("12345678D")
                    .build();

            Patient noBirthDatePatient = new Patient();
            noBirthDatePatient.setId(UUID.randomUUID());
            noBirthDatePatient.setName("Jane");
            noBirthDatePatient.setLastName("NoBirthDate");
            noBirthDatePatient.setPhone("123456789");
            noBirthDatePatient.setDni("12345678D");

            when(patientService.create(any(PatientCreateDto.class))).thenReturn(patientDtoMapper.toDto(noBirthDatePatient));

            mockMvc.perform(post("/api/patients")
                            .contentType(MediaType.APPLICATION_JSON)
                            .content(objectMapper.writeValueAsString(noBirthDateDto)))
                    .andExpect(status().isCreated())

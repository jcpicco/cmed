...
import com.medcentermanager.app.mappers.notes.MedicalRecordNoteCreateDtoMapper;
import com.medcentermanager.app.mappers.notes.MedicalRecordNoteCreateDtoMapperImpl;
...
class MedicalRecordNoteControllerTest {
    ...
    @Nested
    @DisplayName("GET /api/medical-records/{medicalRecordId}/notes - Get All Notes")
    class GetAllNotesTests {
        ...
        void getAllNotes_whenNotesExist_returnsList() throws Exception {
            ...
            mockMvc.perform(get("/api/medical-records/{medicalRecordId}/notes", medicalRecord.getId()))
    ...
    class GetNoteByIdTests {
        ...
        void getNoteById_whenValidId_returnsNote() throws Exception {
            ...
            mockMvc.perform(get("/api/medical-records/{medicalRecordId}/notes/{id}", medicalRecord.getId(), noteId))
                    .andExpect(status().isOk())
                    .andExpect(content().contentType(MediaType.APPLICATION_JSON))
        ...
        void getNoteById_whenInvalidUUID_returns400() throws Exception {
            mockMvc.perform(get("/api/medical-records/{medicalRecordId}/notes/{id}", medicalRecord.getId(), "invalid-uuid"))
    ...
    @Nested
    @DisplayName("POST /api/medical-records/{medicalRecordId}/notes - Create Note")
    class CreateNoteTests {
        ...
        void createNote_withValidData_returnsCreatedNote() throws Exception {
            when(noteService.create(eq(medicalRecord.getId()), any(NoteCreateDto.class))).thenReturn(note);
            when(medicalRecordRepository.findById(medicalRecord.getId())).thenReturn(Optional.of(medicalRecord));

            NoteCreateDto noteCreateDto = noteCreateDtoMapper.toDto(note);

            mockMvc.perform(post("/api/medical-records/{medicalRecordId}/notes", medicalRecord.getId())
            ...
            verify(noteService, times(1)).create(eq(medicalRecord.getId()), any(NoteCreateDto.class));
        }
        ...
        void createNote_withEmptyBody_returns400() throws Exception {
            ...
            verify(noteService, never()).create(any(UUID.class), any(NoteCreateDto.class));
        }
        ...
        void createNote_withMinimalFields_returnsCreatedNote() throws Exception {
            ...
            when(noteService.create(eq(medicalRecord.getId()), any(NoteCreateDto.class))).thenReturn(minimalNote);
            when(medicalRecordRepository.findById(medicalRecord.getId())).thenReturn(Optional.of(medicalRecord));

            NoteCreateDto noteCreateDto = noteCreateDtoMapper.toDto(minimalNote);
    ...
    class UpdateNoteTests {
        ...
        void updateNote_withValidData_returnsUpdatedNote() throws Exception {
            ...
            mockMvc.perform(patch("/api/medical-records/{medicalRecordId}/notes/{id}", medicalRecord.getId(), noteId)
                            .contentType(MediaType.APPLICATION_JSON)
    ...
    class DeleteNoteTests {
        ...
        void deleteNote_whenExists_returnsNoContent() throws Exception {
            ...
            mockMvc.perform(delete("/api/medical-records/{medicalRecordId}/notes/{id}", medicalRecord.getId(), noteId))

...
class PatientServiceTest {
    ...
    @Test
    void create_whenPhoneAndBirthDateAreNull_returnsSavedPatient() {
        PatientCreateDto patientCreateDto = PatientCreateDto.builder()
                .email(patient.getEmail())
                .name(patient.getName())
                .lastName(patient.getLastName())
        ...
        Patient patientWithoutOptionalFields = patient.toBuilder()
                .phone(null)
                .birthDate(null)
                .build();

        PatientDto patientDto = new PatientDto(
                patientWithoutOptionalFields.getId(),
                patientWithoutOptionalFields.getCreatedAt(),
                patientWithoutOptionalFields.getUpdatedAt(),
                patientWithoutOptionalFields.getEmail(),
                patientWithoutOptionalFields.getName(),
                patientWithoutOptionalFields.getLastName(),
                patientWithoutOptionalFields.getSecondLastName(),
                patientWithoutOptionalFields.getPhone(),
                patientWithoutOptionalFields.getDni(),
                patientWithoutOptionalFields.getBirthDate(),
                patientWithoutOptionalFields.getAllergies(),
                Collections.emptyList()
        );

        when(patientCreateDtoMapper.toEntity(patientCreateDto)).thenReturn(patientWithoutOptionalFields);
        when(patientRepository.save(any(Patient.class))).thenReturn(patientWithoutOptionalFields);
        when(patientDtoMapper.toDto(any(Patient.class))).thenReturn(patientDto);

...
class PatientNoteControllerTest {
    ...
    class CreateNoteTests {
        ...
        void createNote_withValidData_returnsCreatedNote() throws Exception {
            when(noteService.create(eq(patient.getId()), any(NoteCreateDto.class))).thenReturn(note);
            when(patientRepository.findById(patient.getId())).thenReturn(Optional.of(patient));

            NoteCreateDto noteCreateDto = NoteCreateDto.builder()
            ...
            verify(noteService, times(1)).create(eq(patient.getId()), any(NoteCreateDto.class));
        }
